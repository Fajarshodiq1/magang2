...
{
    /**
     * Generate comprehensive document hash
     * Hash berdasarkan: file content + metadata + timestamp
     */
    public function generateDocumentHash(Document $document): string
    {
        ...
        return hash('md5', serialize($signatureData));
    }

    /**
     * Generate hash untuk file content saja (untuk perbandingan)
     */
    ...
    /**
     * Generate MD5 checksum untuk file (deteksi perubahan cepat)
     */
    ...
    /**
     * Generate multi-layer hash untuk keamanan maksimal
     */
    public function generateMultiLayerHash(Document $document): array
    {
        return [
            'document_hash' => $this->generateDocumentHash($document),
            'file_hash' => $this->generateFileContentHash($document),
            'checksum' => $this->generateFileChecksum($document->file_path),
            'metadata_hash' => $this->generateMetadataHashOnly($document),
    ...
    public function embedQRToDocument(Document $document): string
    {
        try {
            ...
            if ($fileType === 'pdf') {
                $result = $this->embedQRToPDF($document, $qrCodePath);
            } else {
                $result = $this->createPDFWrapperWithQR($document, $qrCodePath);
            }

            Log::info("QR successfully embedded. Output: {$result}");
    ...
    private function embedQRToPDF(Document $document, string $qrCodePath): string
    {
        try {
            ...
            try {
                ...
                throw new \Exception("Gagal membaca PDF original: " . $e->getMessage() . ". File mungkin corrupt atau terenkripsi.");
            }
            ...
            for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                ...
                try {
                    ...
                    if ($pageNo === 1) {
                        Log::info("Adding QR Code to first page");

                        // QR kecil di pojok kanan bawah, tanpa border/teks
                        $qrSize = 15; // mm
                        $margin  = 4; // mm dari tepi

                        $qrX = $pageWidth  - $qrSize - $margin;
                        $qrY = $pageHeight - $qrSize - $margin;

                        Log::info("QR position: X={$qrX}, Y={$qrY}");
                        ...
                        try {
                            ...
                            Log::info("QR Code image added successfully");
                        } catch (\Exception $e) {
                            Log::error("Failed to add QR image: " . $e->getMessage());
                            throw new \Exception("Gagal menambahkan QR Code image: " . $e->getMessage());
                        }

                        Log::info("QR Code added successfully");
                    }
                } catch (\Exception $e) {
            ...
            if ($fileSize < 1000) {
                throw new \Exception("Generated PDF file is too small ({$fileSize} bytes). Probably corrupt.");
            }

            Log::info("QR Code embedded successfully to PDF: {$newFileName}");
    ...
    private function createPDFWrapperWithQR(Document $document, string $qrCodePath): string
    {
        try {
            ...
            if ($document->archiveClassification) {
                ...
                $pdf->SetFont('helvetica', 'B', 11);
                $pdf->MultiCell(0, 7, $document->archiveClassification->code . ' - ' . $document->archiveClassification->name, 0, 'L');
            }

            $pdf->Ln(12);

            // QR Code — center, tanpa border/judul/teks
            $qrSize   = 25;
            $pageWidth = $pdf->getPageWidth();
            $qrX = ($pageWidth - $qrSize) / 2;
            $qrY = $pdf->GetY();

            $pdf->Image($qrImagePath, $qrX, $qrY, $qrSize, $qrSize, 'PNG', '', '', false, 300, '', false, false, 0, false, false, false);
            ...
            $pdf->Cell(0, 5, 'Document Management System - Verification Certificate', 0, 0, 'C');
    ...
    /**
     * Verifikasi hash dokumen dengan logging detail
     */
    public function verifyDocumentHash(Document $document): array
    {
        ...
        try {
            // 1. Document Hash
            $currentDocumentHash = $this->generateDocumentHash($document);
            $results['document_hash_valid'] = ($currentDocumentHash === $document->document_hash);

            Log::debug('VERIFY document_hash', [
                'doc_id'   => $document->id,
                'stored'   => $document->document_hash,
                'current'  => $currentDocumentHash,
                'match'    => $results['document_hash_valid'],
            ]);

            if (!$results['document_hash_valid']) {
                $results['changes_detected'][] = 'Document signature berubah';
                $results['messages'][] = 'PERINGATAN: Dokumen atau metadata telah dimodifikasi!';
            }

            // 2. File Content Hash
            if ($document->file_content_hash) {
                $currentFileHash = $this->generateFileContentHash($document);
                $results['file_hash_valid'] = ($currentFileHash === $document->file_content_hash);

                Log::debug('VERIFY file_content_hash', [
                    'doc_id'  => $document->id,
                    'stored'  => $document->file_content_hash,
                    'current' => $currentFileHash,
                    'match'   => $results['file_hash_valid'],
                ]);

                if (!$results['file_hash_valid']) {
                    $results['changes_detected'][] = 'Konten file berubah';
                    $results['messages'][] = 'CRITICAL: Isi file telah diubah!';
                }
            } else {
                $results['file_hash_valid'] = true;
                Log::debug('VERIFY file_content_hash: NOT SET, skipping', ['doc_id' => $document->id]);
            }

            // 3. Checksum
            if ($document->file_checksum) {
                $currentChecksum = $this->generateFileChecksum($document->file_path);
                $results['checksum_valid'] = ($currentChecksum === $document->file_checksum);

                Log::debug('VERIFY checksum', [
                    'doc_id'  => $document->id,
                    'stored'  => $document->file_checksum,
                    'current' => $currentChecksum,
                    'match'   => $results['checksum_valid'],
                ]);

                if (!$results['checksum_valid']) {
                    $results['changes_detected'][] = 'File checksum tidak cocok';
                }
            } else {
                $results['checksum_valid'] = true;
                Log::debug('VERIFY checksum: NOT SET, skipping', ['doc_id' => $document->id]);
            }
            ...
            if ($results['valid']) {
                $results['messages'][] = 'Dokumen asli dan tidak ada perubahan';
            }
    ...
    public function generateSecurityFeatures(Document $document): bool
    {
        DB::beginTransaction();
        try {
            Log::info('Generating security features for new document ID: ' . $document->id);
            ...
            DB::commit();
            Log::info('Security features generated successfully for document ID: ' . $document->id);
    ...
    /**
     * Regenerate security features (jika ada perubahan)
     */
    public function regenerateSecurity(Document $document): bool
    {
        DB::beginTransaction();
        try {
            Log::info('Regenerating security for document ID: ' . $document->id);
            ...
            $hasChanges = ($oldHashes['document_hash'] !== $newHashes['document_hash']) ||
                ($oldHashes['file_content_hash'] !== $newHashes['file_hash']) ||
                ($oldHashes['file_checksum'] !== $newHashes['checksum']);
            ...
            if ($hasChanges) {
                DocumentHashHistory::create([
                    ...
                    'is_valid' => false,
                    'changes_detected' => json_encode(['File atau metadata berubah saat regenerasi']),
                    'verified_by' => auth()->id(),
                    'verified_at' => now(),
                    'ip_address' => request()->ip(),
                ]);

                Log::warning('Document ID ' . $document->id . ' has changes during regeneration');
            }
            ...
            $this->logAccess(
                $document,
                'regenerate_security',
                $hasChanges ? 'Security regenerated with changes detected' : 'Security regenerated successfully'
            );

            DB::commit();
            Log::info('Security regenerated successfully for document ID: ' . $document->id);
